...
def forecast_main():
    ...
    pre = model.predict(data_la1)
    # pre = pre * (max_value1 - min_value1) + min_value1
    # y_test = y_test * (max_value1 - min_value1) + min_value1
    pre_max = np.max(pre)
    # date_start, date_end = '2020-01-11', '2021-06-24'
    # Time_list = nday_list(date_start, date_end).tolist()
    # len_T = -(dt.datetime.strptime(date_start, '%Y-%m-%d') - dt.datetime.strptime(date_end, '%Y-%m-%d')).days
    ...
    plt.plot(data_Y[train_size: total_size - rolling_size], predic, 'b', label='predict')
